    ...
    for case_name in ['holland', 'netherlands']:
        ...
        for temperature in [100, 200, 500]:
            for cooling_scheme in ['exponential', 'logaritmic', 'lineair']:
                ...
                lines_comparison_long_experiments_hc(results_dict, f"../../docs/graphs/annealing/2000/annealing_{temperature}_{cooling_scheme}_{case_name}", f"annealing {temperature} {cooling_scheme}", case_name)
...
def make_plots_hill_climber():
    ...
    for algorithm in ['hill_climber', 'hill_climber_restart']:
        for case_name in ['holland', 'netherlands']:
            ...
            for start in starts:
                for mutation in ["light", "heavy"]:
                    onegrid_hc(results_dict, f"../../docs/graphs/{algorithm}/line_{algorithm}_{case_name}_{start}_{mutation}", f"{algorithm} {case_name} {start} {mutation}", start, mutation, 'linediagram')
                    onegrid_hc(results_dict, f"../../docs/graphs/{algorithm}/histo_{algorithm}_{case_name}_{start}_{mutation}", f"{algorithm} {case_name} {start} {mutation}", start, mutation, 'histogram')
        ...
        for generation_count in 200:
            for max_runners in [3, 15]:
                for heuristic in [0, 1, 2]:
                    onegrid_ppa(results_dict, f"../../docs/graphs/ppa/line_{case_name}_{starting_states}_{filter_method}_{population_size}_{generation_count}_{max_runners}_{heuristic}", f"ppa {case_name} {starting_states} {filter_method} {population_size} {generation_count} {max_runners}", population_size, generation_count, max_runners, 'linediagram', heuristic)
                    onegrid_ppa(results_dict, f"../../docs/graphs/ppa/histo_{case_name}_{starting_states}_{filter_method}_{population_size}_{generation_count}_{max_runners}_{heuristic}", f"ppa {case_name} {starting_states} {filter_method} {population_size} {generation_count} {max_runners}", population_size, generation_count, max_runners, 'histogram', heuristic)
