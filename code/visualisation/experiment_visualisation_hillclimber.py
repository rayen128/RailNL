...
def line_diagram_multiple_lines_hillclimber_onegrid_short_experiments(results_dict: dict, export_file_path: str, title_diagram: str, start: str, mutation: str):
    ...
    filtered_results_mutation = filter_states(
        filtered_results_start, 'mutation', mutation)
    ...
    make_line_diagram_multiple_lines(
        scores_lists, title_diagram, export_file_path, False)
...
def line_diagram_multiple_lines_hillclimber_comparison_short_experiments(results_dict: dict, export_file_path: str, title_diagram: str):
    ...
    for start in start_parameters:
        ...
        for mutation in mutation_parameters:
            filtered_results = filter_states(
                filtered_results_start, 'mutation', mutation)
            sorted_results = dict(sorted(filtered_results.items(
            ), key=lambda x: float(x[1]['score']), reverse=True))
    ...
    make_line_diagram_multiple_lines(
        best_scores_lists, title_diagram, export_file_path, True)
...
def line_diagram_multiple_lines_hillclimber_comparison_long_experiments(results_dict: dict, export_file_path: str, title_diagram: str):
    ...
    make_line_diagram_multiple_lines(
        best_scores_lists, title_diagram, export_file_path, True, legend)


if __name__ == "__main__":
    results_dict = read_csv(
        "../../data/hill_climber/experiment_hill_climber_grid_search_holland.csv", 'run_id')
    ...
    line_diagram_multiple_lines_hillclimber_comparison_long_experiments(
        results_dict, "hill_climber_random_heavy", "test line diagram experiment")
